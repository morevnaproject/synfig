...
TransformStack::perform(const synfig::Vector& x)const
{
	...
	if (0) printf("   PERFORM %s: %5.2f %5.2f", get_guid().get_string().substr(0,6).c_str(), ret[0], ret[1]);
	for(const_reverse_iterator iter(rbegin());iter!=rend();++iter)
	{
		ret=(*iter)->perform(ret);
		if (0) printf(" (%14s) %5.2f %5.2f", (*iter)->get_string().c_str(), ret[0], ret[1]);
	}
	if (0) printf("\n");
...
TransformStack::unperform(const synfig::Vector& x)const
{
	...
	if (0) printf(" UNPERFORM %s: %5.2f %5.2f", get_guid().get_string().substr(0,6).c_str(), ret[0], ret[1]);
	for(const_iterator iter(begin());iter!=end();++iter)
	{
