...
{
	if (0) printf("%s:%d parse_linkable_value_node\n", __FILE__, __LINE__);
	...
	if (0) printf("%s:%d parse_linkable_value_node done\n", __FILE__, __LINE__);
	return value_node;
}
...
{
	if (0) printf("%s:%d parse_value_node\n", __FILE__, __LINE__);
		...
		if(value_node)
		{
			...
			if(element->get_name()!="canvas" && ValueBase::ident_type(element->get_name()) != type_nil)
			{
				if (element->get_name() == "bone_valuenode")
				{
					...
					if (!value_node_bone)
					{
						if (0) printf("%s:%d bone_valuenode isn't a ValueNode_Bone?  It's a placeholder?\n", __FILE__, __LINE__);
						return value_node;
					}
	...
	if (0) printf("%s:%d element name = '%s'\n", __FILE__, __LINE__, element->get_name().c_str());
	if(element->get_name()!="canvas" && ValueBase::ident_type(element->get_name()) != type_nil)
	{
		if (0) printf("%s:%d parse_value_node calls parse_value\n", __FILE__, __LINE__);
	...
	{
		if (0) printf("%s:%d parse_value_node calls parse_animated\n", __FILE__, __LINE__);
		value_node=parse_animated(element,canvas);
	}
	else
	if(element->get_name()=="static_list")
	{
		if (0) printf("%s:%d parse_value_node calls parse_static_list\n", __FILE__, __LINE__);
		value_node=parse_static_list(element,canvas);
	}
	else
	if(element->get_name()=="dynamic_list")
	{
		if (0) printf("%s:%d parse_value_node calls parse_dynamic_list\n", __FILE__, __LINE__);
	...
	{
		if (0) printf("%s:%d parse_value_node calls parse_dynamic_list for bline\n", __FILE__, __LINE__);
		value_node=parse_dynamic_list(element,canvas);
	}
	else
	...
	{
		if (0) printf("%s:%d parse_value_node calls parse_linkable_value_node\n", __FILE__, __LINE__);
		value_node=parse_linkable_value_node(element,canvas);
		if (!value_node)
		{
			if (0) printf("%s:%d parse_linkable_value_node gave us a null valuenode\n", __FILE__, __LINE__);
	...
	if(element->get_name()=="canvas")
	{
		if (0) printf("%s:%d parse_value_node calls parse_canvas\n", __FILE__, __LINE__);
		value_node=ValueNode_Const::create(parse_canvas(element,canvas,true));
	}
	else
	{
		if (0) printf("%s:%d parse_value_node doesn't know what to call\n", __FILE__, __LINE__);
		error_unexpected_element(element,element->get_name());
	...
	if (0) printf("%s:%d parse_value_node done\n", __FILE__, __LINE__);
	return value_node;
}
...
{
	if (0) printf("%s:%d parse_canvas_defs\n", __FILE__, __LINE__);
...
CanvasParser::parse_canvas_bones(xmlpp::Element *element,Canvas::Handle canvas)
{
	...
	if (0) printf("%s:%d parse_canvas_bones done\n", __FILE__, __LINE__);
	return bone_list;
}
